...
n = size(timeseries_error_rate)
result = zeros(1, n(2));
%{
for i=1:n(2)
    result(i) = sum(timeseries_error_rate(1:i))/i;
end
%}
figure(2)
hold on
xlabel('time points','FontSize',15)
ylabel('error rates','FontSize',15);
title('Average Error Rates with Window Size = 30', 'FontSize',18)
for i=1:n(2)-30
    result(i) = sum(timeseries_error_rate(i:i+30))/30;
end

plot(result(find(result ~= 0)))
grid on
grid minor
hold off
